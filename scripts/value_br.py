...
def read_fundamental_metric(cfg: Config, metric_code: str) -> pd.DataFrame:
    """Return tidy fundamentals dataframe [date, ticker, value] for the given metric."""
    assert os.path.exists(cfg.db_path), f"Database not found: {cfg.db_path}"
    start_pad = None
    if cfg.start_date:
...
def build_ev_ebitda_signal(cfg: Config, calendar_month_index: pd.DatetimeIndex) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    ebitda_df = read_fundamental_metric(cfg, "EBITDA_TTM")
    print(f"  Retrieved {len(ebitda_df):,} EBITDA TTM rows" if not ebitda_df.empty else "  No EBITDA_TTM data found")
    if ebitda_df.empty:
    ...
    ebitda_pivot = ebitda_df.pivot(index="date", columns="ticker", values="value").sort_index()

    # Monthly EV (calendar month-end) using last available observation each month
    ev_monthly = ev_pivot.resample("M").last()

    # Apply reporting lag to EBITDA by shifting the index forward
    lag_months = max(0, int(getattr(cfg, "ebitda_lag_months", 3) or 0))
    lag_offset = pd.offsets.MonthEnd(lag_months)
    ebitda_shifted = ebitda_pivot.copy()
    if lag_months > 0:
        ebitda_shifted.index = ebitda_shifted.index + lag_offset
    else:
        ebitda_shifted.index = ebitda_shifted.index + pd.offsets.MonthEnd(0)

    if not ebitda_pivot.empty:
        src_arr = np.tile(ebitda_pivot.index.to_numpy()[:, None], (1, ebitda_pivot.shape[1]))
        ebitda_source = pd.DataFrame(
            src_arr,
            index=ebitda_pivot.index,
            columns=ebitda_pivot.columns,
        )
        if lag_months > 0:
            ebitda_source.index = ebitda_source.index + lag_offset
        else:
            ebitda_source.index = ebitda_source.index + pd.offsets.MonthEnd(0)
        ebitda_source = ebitda_source.reindex(calendar_month_index).ffill()
    else:
        ebitda_source = pd.DataFrame(index=calendar_month_index, columns=ebitda_pivot.columns, dtype="datetime64[ns]")

    # Reindex both series to calendar month-end index and forward-fill
    ev_monthly = ev_monthly.reindex(calendar_month_index).ffill()
    ebitda_monthly = ebitda_shifted.reindex(calendar_month_index).ffill()
    ...
    ebitda_source = ebitda_source.reindex(columns=signal.columns)

    components = {
        "ev_monthly": ev_pos,
        "ebitda_lagged": ebitda_pos,
        "ev_to_ebitda": ratio,
        "ebitda_source_dates": ebitda_source,
    }
...
def run_analysis(cfg: Config) -> None:
    ...
    if ebitda_source is not None and not ebitda_source.empty:
        month_series = pd.Series(ebitda_source.index, index=ebitda_source.index)
        def _staleness(col: pd.Series) -> pd.Series:
            return (month_series - col).dt.days / 30.4375
        staleness_months = ebitda_source.apply(_staleness)
