#!/usr/bin/env python3
"""
Brazil Value factor backtest.

Implements an EV/EBITDA long/short value strategy reusing the momentum engine's
infrastructure (cohorts, guardrails, overlays, vol targeting) while sourcing
enterprise value and EBITDA fundamentals with a configurable reporting lag.
"""
import argparse
import json
import os
...
from fundamentals import build_lagged_metric, compute_staleness_months, read_fundamental_metric
...
def build_ev_ebitda_signal(cfg: Config, calendar_month_index: pd.DatetimeIndex) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    """Construct EV/EBITDA value signal with reporting lag."""
    print("Loading enterprise value series ...")
    ev_df = read_fundamental_metric(cfg, "ENTERPRISE_VALUE")
    print(f"  Retrieved {len(ev_df):,} enterprise value rows" if not ev_df.empty else "  No enterprise value data found")
    if ev_df.empty:
        raise RuntimeError("ENTERPRISE_VALUE series is empty; cannot build value factor.")

    print("Loading EBITDA TTM series ...")
    lagged_ebitda = build_lagged_metric(
        cfg,
        "EBITDA_TTM",
        calendar_month_index,
        lag_months=getattr(cfg, "ebitda_lag_months", 3),
    )
    if lagged_ebitda.values.empty or lagged_ebitda.values.dropna(how="all").empty:
        raise RuntimeError("EBITDA_TTM series is empty; cannot build value factor.")

    # Monthly EV (calendar month-end) using last available observation each month
    ev_pivot = ev_df.pivot(index="date", columns="ticker", values="value").sort_index()
    ev_monthly = ev_pivot.resample("M").last().reindex(calendar_month_index).ffill()
    ebitda_monthly = lagged_ebitda.values.reindex(calendar_month_index).ffill()

    # Guard: positive values only for ratio/logs
    ev_pos = ev_monthly.where(ev_monthly > 0)
    ebitda_pos = ebitda_monthly.where(ebitda_monthly > 0)

    with np.errstate(divide="ignore", invalid="ignore"):
        log_ev = np.log(ev_pos)
        log_ebitda = np.log(ebitda_pos)
        signal = (log_ebitda - log_ev)
        ratio = ev_pos / ebitda_pos
    ...
    ev_pos = ev_pos.reindex(columns=signal.columns)
    ebitda_pos = ebitda_pos.reindex(columns=signal.columns)
    ratio = ratio.reindex(columns=signal.columns)

    components = {
        "ev_monthly": ev_pos,
        "ebitda_lagged": ebitda_pos,
        "ev_to_ebitda": ratio,
        "ebitda_source_dates": lagged_ebitda.source_dates.reindex(calendar_month_index).reindex(columns=signal.columns),
    }
...
def run_analysis(cfg: Config) -> None:
    out_dir = os.path.abspath(getattr(cfg, "out_dir", "results"))
    ...
    # 2) Value signal on monthly closes
    print("Preparing monthly closes ...")
    monthly_close = pc.resample('M').last()
    print("Building EV/EBITDA value signal with reporting lag ...")
    value_signal, value_components = build_ev_ebitda_signal(cfg, monthly_close.index)
    total_value_names = len(value_signal.columns)
    if total_value_names == 0:
        raise RuntimeError("No tickers with valid EV/EBITDA signal after preprocessing.")
    coverage_counts = value_signal.notna().sum(axis=1)
    value_components["coverage_counts"] = coverage_counts
    if not coverage_counts.empty:
        coverage_ratio = coverage_counts / float(total_value_names)
        coverage_active = coverage_counts[coverage_counts > 0]
        ratio_active = coverage_ratio.loc[coverage_active.index]
        med_cov = float(coverage_active.median()) if not coverage_active.empty else float("nan")
        med_ratio = float(ratio_active.median()) if not ratio_active.empty else float("nan")
        min_cov_all = int(coverage_counts.min())
        min_cov_active = int(coverage_active.min()) if not coverage_active.empty else 0
        latest_cov = int(coverage_counts.iloc[-1])
        latest_ratio = float(coverage_ratio.iloc[-1])
        median_text = f"{med_cov:.0f} ({med_ratio:.1%})" if med_cov == med_cov and med_ratio == med_ratio else "n/a"
        print(f"Value signal coverage: {total_value_names} tickers with EV/EBITDA history; "
              f"median monthly coverage {median_text}, "
              f"min positive {min_cov_active:.0f}, overall min {min_cov_all:.0f}. "
              f"Latest month: {latest_cov} names ({latest_ratio:.1%}).")
        if min_cov_active < cfg.min_eligible and min_cov_active > 0:
            print(f"Warning: EV/EBITDA coverage dips to {min_cov_active} names (< min_eligible={cfg.min_eligible}); "
                  "some months may be skipped.")
        if min_cov_all == 0:
            print("Note: early months have zero EV/EBITDA coverage before fundamentals reach the lagged window.")
    else:
        print("Warning: Coverage series for value signal is empty.")

    ebitda_source = value_components.get("ebitda_source_dates")
    if ebitda_source is not None and not ebitda_source.empty:
        staleness_months = compute_staleness_months(ebitda_source).astype(float)
        staleness_months = staleness_months.replace([np.inf, -np.inf], np.nan)
        value_components["ebitda_staleness_months"] = staleness_months
        stale_flat = staleness_months.stack().dropna()
        if not stale_flat.empty:
            med_stale = float(np.nanmedian(stale_flat))
            p90_stale = float(np.nanpercentile(stale_flat, 90))
            latest_stale = staleness_months.iloc[-1].dropna()
            share_gt18 = float((latest_stale > 18).mean()) if not latest_stale.empty else float("nan")
            share_text = f"{share_gt18:.1%}" if share_gt18 == share_gt18 else "n/a"
            print(f"EBITDA staleness (months): median {med_stale:.1f}, 90th pct {p90_stale:.1f}; "
                  f"latest month share >18m = {share_text}.")
            if p90_stale > 18.0:
                print("Warning: EBITDA data older than 18 months for a meaningful portion of the universe.")
    ...
    for idx_loop, (d_mend, d_trade, d_next_trade) in enumerate(zip(mend_labels, trade_dates, next_trade_dates)):
        ...
        # Value scores for this month (calendar month-end label)
        period = pd.Period(d_mend, freq='M')
        ts_month = period.to_timestamp('M')
        if ts_month not in value_signal.index:
            continue
        signal_row = value_signal.loc[ts_month]
        signal_row = signal_row.dropna()
        signal_row = signal_row.loc[signal_row.index.intersection(eligible)]
        if signal_row.empty:
            continue

        # Sector-neutral z-score within sector, then global ranking by z-score
        if ticker_sector:
            sectors_series = pd.Series({t: ticker_sector.get(t, "Unknown") for t in signal_row.index})
            df_sc = pd.DataFrame({"signal": signal_row, "sector": sectors_series})
            def zscore(g: pd.Series) -> pd.Series:
                mu = g.mean()
                sd = g.std(ddof=0)
                return (g - mu) / sd if sd and np.isfinite(sd) and sd > 1e-12 else g * 0.0
            df_sc["z"] = df_sc.groupby("sector")["signal"].transform(zscore)
            signal_for_rank = df_sc["z"]
        else:
            signal_for_rank = signal_row
        if signal_row.empty:
            continue
        # assign deciles
        try:
            ...
            try:
                if target_rows:
                    ...
                    if "sleeve" not in df_targets.columns:
                        ...
                        df_targets.insert(insert_pos, "sleeve", "value")
                    df_targets["sleeve"] = "value"
                    df_targets.sort_values(["book", "cohort_id", "side", "ticker"], inplace=True)
                    df_targets_sleeve = df_targets.copy()
                    targets_path = os.path.join(out_dir_local, f"targets_{period_label}.csv")
                    if os.path.exists(targets_path):
                        ...
                        existing_targets = existing_targets[existing_targets["sleeve"] != "value"]
                    ...
                    if getattr(cfg, 'live_capital', None) is not None:
                        ...
                        if ord_rows:
                            ...
                            orders_path = os.path.join(out_dir_local, f"orders_{period_label}.csv")
                            if os.path.exists(orders_path):
                                ...
                                existing_orders = existing_orders[existing_orders["sleeve"] != "value"]
                            ...
                            try:
                                ...
                                detail_path = os.path.join(out_dir_local, f"account_targets_{period_label}_by_sleeve.csv")
                                aggregate_path = os.path.join(out_dir_local, f"account_targets_{period_label}.csv")
                                ...
                                orders_live_path = os.path.join(out_dir_local, f"orders_live_{period_label}.csv")
                                orders_live_detail_path = os.path.join(out_dir_local, f"orders_live_{period_label}_by_sleeve.csv")
                                if ord_detail_rows:
                                ...
                                if getattr(cfg, 'cold_start', False):
                                    try:
                                        ...
                                        if 'sleeve' not in cold.columns:
                                            cold.insert(2, 'sleeve', "value")
                                        else:
                                            cold['sleeve'] = "value"
                                        df_account_targets_cs_new = cold.loc[:, cols].sort_values(["book", "ticker"])  # type: ignore
                                        cs_detail_path = os.path.join(out_dir_local, f"account_targets_coldstart_{period_label}_by_sleeve.csv")
                                        cs_agg_path = os.path.join(out_dir_local, f"account_targets_coldstart_{period_label}.csv")
                                        ...
                                        cs_orders_detail_path = os.path.join(out_dir_local, f"orders_live_coldstart_{period_label}_by_sleeve.csv")
                                        cs_orders_path = os.path.join(out_dir_local, f"orders_live_coldstart_{period_label}.csv")
                                ...
                                if alloc_rows:
                                    ...
                                    df_alloc.to_csv(os.path.join(out_dir_local, f"allocations_{period_label}.csv"), index=False)
                            except Exception:
    ...
    # Financing adjustments: CDI carry on short cash collateral and borrow cost on shorts
    ...
    borrow_ann = max(borrow_ann, -0.99)
    ...
    if "LS" in pf.columns:
        ...
        if len(ls_series) >= 12:
            ...
            if risk_summary["max_drawdown_12m"] <= -0.08:
                alerts.append(f"Value 12m max drawdown {risk_summary['max_drawdown_12m']:.2%} exceeds 8% limit")
        if len(ls_series) >= 24:
        ...
        if risk_summary["worst_month"] <= -0.06:
            alerts.append(f"Value worst monthly LS return {risk_summary['worst_month']:.2%} worse than -6%")
    if "risk_ls_weighted_multiplier" in pf.columns:
        ...
        if np.isfinite(risk_summary.get("last_ls_multiplier", np.nan)) and risk_summary["last_ls_multiplier"] <= 0.6:
            alerts.append(f"Value latest LS risk multiplier {risk_summary['last_ls_multiplier']:.2f} below 0.60")
        if np.isfinite(risk_summary.get("last_ls_high", np.nan)) and risk_summary["last_ls_high"] >= 5:
            alerts.append(f"Value has {int(risk_summary['last_ls_high'])} LS names flagged high risk this month")

    if risk_summary:
        risk_summary_path = os.path.join(out_dir, "value_br_risk_summary.json")
    ...
    an = results.get("LS_net_alpha_vs_BOVA11", {})
    if an:
        print("  LS (Net) Alpha: {} bps/month (t={:.2f}), Beta={:.3f}, IR(ann)={:.2f}".format(
            an.get('alpha_bps_per_month', float('nan')),
            an.get('alpha_tstat', float('nan')),
            an.get('beta', float('nan')),
            an.get('ir_ann', float('nan')),
        ))
    ...
    print("\nTiming hygiene: Value spec (EV/EBITDA with reporting lag) forms portfolios at calendar month-end using data through t, trades at next day's open (T+1), and holds to next T+1 open. All reported t-stats use Newey-West (lags={}).".format(cfg.nw_lags))
    ...
    alpha_base = results.get("LS_net_alpha_vs_BOVA11") or results.get("LS_alpha_vs_BOVA11", {})
    ce_alpha_bps = alpha_base.get("alpha_bps_per_month", np.nan) if isinstance(alpha_base, dict) else np.nan
    ...
    if kill_triggered:
        ...
        # Save and exit early
        pf.to_csv(os.path.join(out_dir, "value_br_timeseries.csv"))
        pd.Series(results).to_json(os.path.join(out_dir, "value_br_summary.json"))
        pd.DataFrame(grid_results).to_csv(os.path.join(out_dir, "value_br_cost_grid.csv"), index=False)
        return
...
def make_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Brazil Value Factor Backtest (EV/EBITDA, monthly, LS)")
    ...
    p.add_argument("--ebitda-lag-months", type=int, default=3,
                   help="Number of months to delay EBITDA TTM availability (e.g., 3 = first use occurs 3 months after period date)")
...
def config_from_args(args: argparse.Namespace, *, start_date: Optional[str] = None,
    ...
    return Config(
        ...
        ebitda_lag_months=getattr(args, "ebitda_lag_months", 3),
        ...
        micro_add_frac=getattr(args, "micro_add_frac", Config.micro_add_frac),
