#!/usr/bin/env python3
"""
Brazil Quality factor backtest.

Implements a quality-driven long/short strategy reusing the momentum engine's
infrastructure (cohorts, guardrails, overlays, vol targeting) while sourcing
ROE/ROA/gross-margin/EBIT-to-debt fundamentals with a configurable reporting lag.
"""
import argparse
import os
import sqlite3
...
from fundamentals import build_lagged_metrics, compute_staleness_months
...
def build_quality_signal(cfg: Config, calendar_month_index: pd.DatetimeIndex) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    """Construct a quality score from lagged profitability and balance-sheet metrics."""
    lag_months = getattr(cfg, "quality_lag_months", 3)
    print("Loading quality fundamentals ...")
    lagged = build_lagged_metrics(cfg, QUALITY_METRICS, calendar_month_index, lag_months=lag_months)

    aligned_values: Dict[str, pd.DataFrame] = {}
    aligned_sources: Dict[str, pd.DataFrame] = {}
    for code, series in lagged.items():
        values = series.values.reindex(calendar_month_index).ffill()
        sources = series.source_dates.reindex(calendar_month_index)
        if values.empty or values.dropna(how="all").empty:
            print(f"  Warning: {code} metric has no data after lag; skipping in score.")
            continue
        aligned_values[code] = values.astype(float)
        aligned_sources[code] = sources.reindex(columns=values.columns)

    if not aligned_values:
        raise RuntimeError("No quality metrics available after applying reporting lag.")

    all_columns = sorted(set().union(*(df.columns for df in aligned_values.values())))
    quality_sum = None
    quality_count = None
    components: Dict[str, pd.DataFrame] = {}

    for code, values in aligned_values.items():
        df = values.reindex(columns=all_columns)
        mean = df.mean(axis=1)
        std = df.std(axis=1, ddof=0).replace(0.0, np.nan)
        z_scores = df.sub(mean, axis=0).div(std, axis=0)
        if quality_sum is None:
            quality_sum = z_scores.copy()
            quality_count = (~z_scores.isna()).astype(float)
        else:
            quality_sum = quality_sum.add(z_scores, fill_value=0.0)
            quality_count = quality_count.add((~z_scores.isna()).astype(float), fill_value=0.0)
        components[f"{code.lower()}_values"] = df
        components[f"{code.lower()}_source_dates"] = aligned_sources.get(code, pd.DataFrame(index=calendar_month_index, columns=all_columns))

    quality_score = quality_sum.divide(quality_count).replace([np.inf, -np.inf], np.nan)
    quality_score = quality_score.dropna(axis=1, how="all")
    components["quality_inputs_count"] = quality_count.reindex(columns=quality_score.columns)
    components["quality_zsum"] = quality_sum.reindex(columns=quality_score.columns)
...
def run_analysis(cfg: Config) -> None:
    ...
    # 2) Quality signal on monthly closes
    print("Preparing monthly closes ...")
    monthly_close = pc.resample('M').last()
    print("Building quality score from fundamentals ...")
    quality_signal, quality_components = build_quality_signal(cfg, monthly_close.index)
    total_quality_names = len(quality_signal.columns)
    if total_quality_names == 0:
        raise RuntimeError("No tickers with valid Quality signal after preprocessing.")
    coverage_counts = quality_signal.notna().sum(axis=1)
    quality_components["coverage_counts"] = coverage_counts
    if not coverage_counts.empty:
        coverage_ratio = coverage_counts / float(total_quality_names)
        ...
        print(f"Quality signal coverage: {total_quality_names} tickers with quality fundamentals (ROE/ROA/GrossMargin/EBIT-Debt); "
        ...
        if min_cov_active < cfg.min_eligible and min_cov_active > 0:
            print(f"Warning: Quality coverage dips to {min_cov_active} names (< min_eligible={cfg.min_eligible}); "
                  "some months may be skipped.")
        if min_cov_all == 0:
            print("Note: early months have zero quality coverage before fundamentals reach the lagged window.")
    else:
        print("Warning: Coverage series for quality signal is empty.")

    for metric_code in QUALITY_METRICS:
        key = f"{metric_code.lower()}_source_dates"
        src = quality_components.get(key)
        if src is None or src.empty:
            continue
        staleness = compute_staleness_months(src).astype(float)
        staleness = staleness.replace([np.inf, -np.inf], np.nan)
        quality_components[f"{metric_code.lower()}_staleness_months"] = staleness
        stale_flat = staleness.stack().dropna()
        if stale_flat.empty:
            continue
        med_stale = float(np.nanmedian(stale_flat))
        p90_stale = float(np.nanpercentile(stale_flat, 90))
        latest_stale = staleness.iloc[-1].dropna()
        ...
        print(f"{metric_code} staleness (months): median {med_stale:.1f}, 90th pct {p90_stale:.1f}; "
              f"latest month share >18m = {share_text}.")
        if p90_stale > 18.0:
            print(f"Warning: {metric_code} data older than 18 months for a meaningful portion of the universe.")
    ...
    for idx_loop, (d_mend, d_trade, d_next_trade) in enumerate(zip(mend_labels, trade_dates, next_trade_dates)):
        ...
        if ts_month not in quality_signal.index:
            continue
        signal_row = quality_signal.loc[ts_month]
        ...
        if use_staggered:
            ...
            try:
                if target_rows:
                    ...
                    df_targets.to_csv(os.path.join(out_dir_local, f"targets_{period_label}.csv"), index=False)
                    ...
                    if getattr(cfg, 'live_capital', None) is not None:
                        ...
                        if ord_rows:
                            pd.DataFrame(ord_rows).sort_values(["book", "cohort_id", "ticker"]).to_csv(
                                os.path.join(out_dir_local, f"orders_{period_label}.csv"), index=False
                            )
                            ...
                            try:
                                ...
                                df_account_targets.to_csv(os.path.join(out_dir_local, f"account_targets_{period_label}.csv"), index=False)
                                ...
                                if ord_agg_rows:
                                    ...
                                    df_orders_live.to_csv(os.path.join(out_dir_local, f"orders_live_{period_label}.csv"), index=False)
                                ...
                                if getattr(cfg, 'cold_start', False):
                                    try:
                                        ...
                                        df_account_targets_cs.to_csv(os.path.join(out_dir_local, f"account_targets_coldstart_{period_label}.csv"), index=False)
                                        ...
                                        if ord_cs_rows:
                                            ...
                                            df_orders_live_cs.to_csv(os.path.join(out_dir_local, f"orders_live_coldstart_{period_label}.csv"), index=False)
                                    except Exception:
    ...
    if "LS" in pf.columns:
        ...
        if len(ls_series) >= 12:
            ...
            if risk_summary["max_drawdown_12m"] <= -0.08:
                alerts.append(f"Quality 12m max drawdown {risk_summary['max_drawdown_12m']:.2%} exceeds 8% limit")
        if len(ls_series) >= 24:
        ...
        if risk_summary["worst_month"] <= -0.06:
            alerts.append(f"Quality worst monthly LS return {risk_summary['worst_month']:.2%} worse than -6%")
    if "risk_ls_weighted_multiplier" in pf.columns:
        ...
        if np.isfinite(risk_summary.get("last_ls_multiplier", np.nan)) and risk_summary["last_ls_multiplier"] <= 0.6:
            alerts.append(f"Quality latest LS risk multiplier {risk_summary['last_ls_multiplier']:.2f} below 0.60")
        if np.isfinite(risk_summary.get("last_ls_high", np.nan)) and risk_summary["last_ls_high"] >= 5:
            alerts.append(f"Quality has {int(risk_summary['last_ls_high'])} LS names flagged high risk this month")

    if risk_summary:
        risk_summary_path = os.path.join(out_dir, "quality_br_risk_summary.json")
    ...
    print("\nTiming hygiene: Quality spec forms portfolios at calendar month-end using data through t, trades at next day's open (T+1), and holds to next T+1 open. All reported t-stats use Newey-West (lags={}).".format(cfg.nw_lags))
    ...
    if kill_triggered:
        ...
        # Save and exit early
        pf.to_csv(os.path.join(out_dir, "quality_br_timeseries.csv"))
        pd.Series(results).to_json(os.path.join(out_dir, "quality_br_summary.json"))
        pd.DataFrame(grid_results).to_csv(os.path.join(out_dir, "quality_br_cost_grid.csv"), index=False)
        return
...
def make_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Brazil Quality Factor Backtest (monthly, LS)")
    ...
    p.add_argument("--quality-lag-months", type=int, default=3,
                   help="Number of months to delay quality fundamentals (e.g., 3 = first use occurs 3 months after period date)")
...
def config_from_args(args: argparse.Namespace, *, start_date: Optional[str] = None,
    ...
    return Config(
        ...
        quality_lag_months=getattr(args, "quality_lag_months", 3),
